...
if @hotel_location ='VN'
		begin  
		insert into [BAO].[bhv_hotelchain].[dbo].[Hotel] values (@hotel_id
															,@hotel_name
															,@hotel_type
															,@hotel_location)
		end
...
as
begin 
	insert into [BAO].[bhv_hotelchain].[dbo].[Customer] values (@id, @name, @mobile, @email)
end
...
begin
	if @hotel_id LIKE 'VN%'
		begin  
		insert into [BAO].[bhv_hotelchain].[dbo].[Room] values (@hotel_id
															,@room_id
															,@type
															,@status
															,@price)
...
as
begin
			insert into [dbo].[Customer] values select * from [BAO].[bhv_hotelchain].[dbo].[Customer] C where C.customer_id = @customerid
			
		if (@hotelid like 'VN%')
		begin  
		insert into [BAO].[bhv_hotelchain].[dbo].[Hotel] values (@hotelid
																, @bookingid
																, @roomid
																, @customerid
																, @checkindate
																...
																, @bookingtype
																, @totalpayment)
		update [BAO].[bhv_hotelchain].[dbo].Room
			set room_status = 0
...
begin
	...
	if(@roomid like 'VN%')
		begin
				select @check= Room_status from [BAO].[bhv_hotelchain].[dbo].[Room] where @roomid = Room.room_id
	if (@check=1)
	...
	if (@check=1)
		...
		if(@roomid like 'UK%')
		begin
				select @check= Room_status from [dbo].[Room] where Room.room_id =  @roomid
	if (@check=1)
			exec addBooking @hotelid, @bookingid, @roomid, @customerid,
		@checkindate ,
		@checkoutdate ,
...
begin
	if(@roomid like 'VN%')
		begin 
		update [BAO].[bhv_hotelchain].[dbo].[Booking]
	set total_payment  =  datediff(d, @checkindate, @checkoutdate) * (select R.room_price from [BAO].[bhv_hotelchain].[dbo].[Room] R join 
															  [BAO].[bhv_hotelchain].[dbo].Booking B on R.room_id = B.room_id) 
	update [BAO].[bhv_hotelchain].[dbo].Room
	set room_status = 1
	...
	set total_payment  =  datediff(d, @checkindate, @checkoutdate) * (select R.room_price from [dbo].[Room] R join 
															 [dbo].Booking B on R.room_id = B.room_id) 
	update   [dbo].Room
	set room_status = 1
